const { RegistrarSetor } = require("../model/RegistrerSetorModel"); // Importa a classe 'Registrar' do arquivo 'RegistrerModel.js'.

exports.index = (req, res) => {
  res.render("registrarsetor"); // Renderiza a view "registrar" ao acessar a rota correspondente.
};

exports.register = async function (req, res) {
  try {
    const userreg = new RegistrarSetor(req.body); // Cria uma instância da classe 'Registrar' passando os dados da requisição (req.body).
    ...
    if (userreg.erros.length > 0) {
      ...
      req.session.save(function () {
        return res.redirect("/registersetor.admin"); // Redireciona de volta para a página de registro.
      });
      return;
    }
